...
class RebaScoreMIT:
    ...
    def init_table_b(self):
        '''
        Table used to compute lower body score
    ...
    def compute_score_a(self):
        ...
        if 0 <= self.angle_dict['trunk_angle'] <= 1:
            ...
            trunk_score +=2
        elif 20 <= self.angle_dict['trunk_angle'] <= 60:
            trunk_score +=3
        elif self.angle_dict['trunk_angle'] > 60:
            trunk_score +=4
        # Trunk adjust
        # trunk_score += 1 if self.angle_dict['trunk_side'] else 0
    ...
    def compute_score_b(self):
        ...
        for i in range(2):
            ...
            # upper_arm_score += 1 if self.angle_dict['arm_abducted'] else 0
            # upper_arm_score -= 1 if self.angle_dict['leaning'] else 0
            ...
            # Wrist position
            if -15 <= self.angle_dict[keys[i][2]] <= 15:
                wrist_score += 1
            else:
                wrist_score += 2

            # # Wrist adjust
            # wrist_score += 1 if self.angle_dict['wrist_twisted'] else 0
            ...
            if i ==0:
                l_score,l_partials = score_b, np.array([upper_arm_score, lower_arm_score, wrist_score])
            else:
                r_score,r_partials = score_b, np.array([upper_arm_score, lower_arm_score, wrist_score])
            
        if l_score>r_score:
            return l_score,l_partials
        else:
            return r_score,r_partials
